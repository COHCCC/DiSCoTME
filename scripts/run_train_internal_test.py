...
try:
    ...
    from src.models.discotme_net import MODELS  # 使用 Registry
...
def parse_args():
    ...
    p.add_argument("--model-arch", type=str, default="standard_discotme", 
                   ...
                   help="Choose model architecture: standard (原版) or factorized (FactorCL)")
...
def build_optimizer(base_model, args, encoder_type="gated"):
    """
    根据不同的 encoder 类型构建 optimizer
    兼容 Standard 和 Factorized 两种模型架构
    """
    param_groups = []
    
    if encoder_type == "gated":
        # ============================================
        # Gated Encoder 的参数分组（DINO 全解冻版本）
        # ============================================
        
        # 1. Image Backbone (DINO) - 全解冻，但学习率低
        param_groups.append({
        ...
        # 2. Image Projection (Encoder 内部的)
        param_groups.append({
        ...
        # Factorized 版：img_proj_shared, img_proj_unique
        ...
        # ============================================
        # GigaPath + Confidence Head 版本（保留旧逻辑）
        # ============================================
        param_groups.append({
...
def main():
    ...
    # ==========================================================
    # Model Initialization (通过 Registry 动态获取)
    # ==========================================================
    ...
    # ==========================================================
    # Training Loop (保持原版逻辑)
    # ==========================================================
    for epoch in range(args.num_epochs):
        sampler.set_epoch(epoch)
        
        # 根据是否使用 distillation 调用不同参数
...
#     base_model = MultiScaleMultiModalModel(
#         img_enc_name=args.image_encoder_type,
#         gene_enc_name=args.gene_encoder_type,
#         proj_dim=args.proj_dim,
#         img_args=img_args,
...
#     if args.run_name is None:
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#         args.run_name = f"{args.image_encoder_type}_{timestamp}"
