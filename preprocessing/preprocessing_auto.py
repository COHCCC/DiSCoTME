...
def print_tiff_info(tif_path: str):
    ...
    with tifffile.TiffFile(tif_path) as tif:
        ...
        print(f"[tifffile] tiled: {page.is_tiled}, compression: {page.compression}")
        try:
            print(f"[tifffile] dtype: {series.dtype}")
        except Exception:
            pass
...
def load_wsi_into_memory(tif_path: str):
    """
    一次性将整张 WSI 读入内存。
    对于 non-tiled TIFF，这比反复读小块快得多。
    """
    print(f"Loading entire WSI into memory: {tif_path}")
    with tifffile.TiffFile(tif_path) as tif:
        wsi_array = tif.asarray()
    
    # 处理通道维度
    if wsi_array.ndim == 3 and wsi_array.shape[0] in (3, 4) and wsi_array.shape[0] < wsi_array.shape[-1]:
        wsi_array = np.moveaxis(wsi_array, 0, -1)  # (C,H,W) -> (H,W,C)
    
    print(f"WSI loaded: shape={wsi_array.shape}, dtype={wsi_array.dtype}, size={wsi_array.nbytes / 1e9:.2f} GB")
    return wsi_array
...
def produce_test_data(
    ...
    # === 4) read in spatial gene expression ===
    ...
    # === 5) extract patch + genes + write metadata ===
    metadata_rows = []
    skipped = 0
    ...
    for idx, (barcode, x, y) in enumerate(rows):
        ...
        if (idx + 1) % 500 == 0 or idx == 0:
            print(f"Processing spot {idx + 1}/{len(rows)}...")
        
        # 5.1) 从内存切 patch
        region_img = read_patch_from_array(wsi_array, x_center=x, y_center=y, radius=radius)
    ...
    print(f"\nDone! Processed {len(metadata_rows)} spots, skipped {skipped}.")
...
def parse_args():
    p = argparse.ArgumentParser(description="Produce test data (fast version - loads WSI into memory).")
    p.add_argument("--root", required=True,
                   help="Path to the Spaceranger outs/ directory")
    p.add_argument("--wsi", default=None,
                   help="Path to WSI (.tif/.tiff). If omitted, will auto-detect.")
    p.add_argument("--radius", type=int, default=None,
                   help="Patch radius in pixels. If omitted, computed from scalefactors.")
